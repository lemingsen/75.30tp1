...
def proporcion_cruzan_campo(grafo, mapper=None):
    ...
    cruzan_bloque = 0
    visitados = set()
    mapper = mapper if mapper is not None else (lambda k: grafo.nodes[k]["type"])
...
def probabilidad_tipos_distintos(grafo, mapper=None):
    """
    :param grafo:
    :param mapper: un mapper de vertice a un tipo (por defecto, grafo[v])
    :return: la probabilidad de que dos vertices unidos por una arista
     sean de tipos distintos
    """
    proporciones_por_tipo = proporcion_por_tipo(grafo, mapper)
    probabilidad = 0
    for proporcion in proporciones_por_tipo.values():
        probabilidad += (1-proporcion)*proporcion
    return probabilidad